...
class NutritionPlanUITests: XCTestCase {
    ...
    func testNutritionPlanComplete() {
        _ = app.wait(for: .runningForeground, timeout: 10)
        app.tabBars["Tab Bar"].buttons["Nutrition"].tap()
        ...
        XCTAssertEqual(tablesQuery.children(matching: .cell).element(boundBy: 0).children(matching: .textField).element(boundBy: 0).value as! String, "Breakfast")
    }
    
    func testNutritionPlanWeek() {
        _ = app.wait(for: .runningForeground, timeout: 10)
        app.tabBars["Tab Bar"].buttons["Nutrition"].tap()
        let tablesQuery = app.tables
        let vukVukasinovicCellsQuery = tablesQuery.cells.containing(.staticText, identifier:"Vuk Vukasinovic")
        ...
        XCTAssertEqual(tablesQuery.children(matching: .cell).element(boundBy: 1).children(matching: .textField).element(boundBy: 0).value as! String, "Lunch")
    }
    
    func testNutritionPlanWeeksLabel() {
        _ = app.wait(for: .runningForeground, timeout: 10)
        app.tabBars["Tab Bar"].buttons["Nutrition"].tap()
        let tablesQuery = app.tables
    ...
    func testCreateNutritionPlan() {
        ...
        app.buttons["Create Nutrition Plan"].tap()
